...
The concurrency experiments reveal non-monotonic scaling patterns that vary by model and operation type. For **Llama-3.2-3B-Instruct**, save times exhibit the following behavior: 245.8 seconds at concurrency=1, 283.2 seconds at concurrency=16 (15 percent slower), and 201.2 seconds at concurrency=128 (18 percent faster than baseline). Load times show consistent improvement: 413.3 seconds at concurrency=1, 297.8 seconds at concurrency=16 (28 percent faster), and 107.9 seconds at concurrency=128 (74 percent faster than baseline).

**OpenLLaMA-3B** demonstrates similar non-monotonic patterns. Save times progress from 323.5 seconds at concurrency=1 to 511.8 seconds at concurrency=16 (58 percent slower), then improve to 211.5 seconds at concurrency=128 (35 percent faster than baseline). Load times show monotonic improvement: 412.9 seconds at concurrency=1, 285.6 seconds at concurrency=16 (31 percent faster), and 61.0 seconds at concurrency=128 (85 percent faster than baseline).

The non-monotonic save time behavior indicates that moderate concurrency levels (16) can introduce overhead that outweighs parallelization benefits. This degradation likely results from thread scheduling overhead, lock contention, or filesystem metadata bottlenecks. At concurrency=128, the system achieves sufficient parallelism to overcome these bottlenecks and realize net performance gains. The magnitude of the concurrency=16 slowdown varies significantly between models (15 percent for Llama-3.2-3B vs 58 percent for OpenLLaMA-3B), suggesting model-specific interactions with the storage subsystem.

Load operations demonstrate substantially greater benefits from increased concurrency than save operations, with improvements of 74-85 percent at concurrency=128 compared to 18-35 percent for saves. This asymmetry indicates that read operations benefit more from parallelization than write operations, likely due to filesystem read-ahead caching and more efficient handling of concurrent read requests. The monotonic improvement in load times across all concurrency levels, contrasting with the non-monotonic save behavior, suggests that read parallelization avoids the contention issues observed during writes.

These results indicate that optimal concurrency settings depend on workload characteristics. For write-intensive workloads, low concurrency (1) may outperform moderate levels (4-16), with high concurrency (128) required to achieve performance gains. For read-intensive workloads, higher concurrency consistently improves performance. The observed benefits at concurrency=128 for 3B models suggest that high parallelism is valuable even for relatively small models.
