...
The concurrency experiments on Llama-3.2-3B-Instruct and OpenLLaMA-3B reveal unexpected non-monotonic scaling patterns where moderate concurrency levels can actually degrade performance compared to sequential operation. Both models show that increasing concurrency from 1 to 16 results in slower save times, with Llama-3.2-3B-Instruct degrading by 15 percent and OpenLLaMA-3B by 58 percent. Only at very high concurrency (128) does performance improve beyond the sequential baseline, achieving modest gains of 18-35 percent for save operations.
...
The chunk size experiments reveal a clear optimal range across all tested models. Very small chunk sizes (1-4 MB) result in significantly degraded performance, while larger chunk sizes (64-256 MB) provide optimal and stable performance. Both save and load times improve dramatically as chunk size increases from 1 MB to 64 MB, then plateau with no further gains beyond 64 MB. This pattern holds consistently across all four models regardless of size or architecture, indicating that **64 MB is a robust default choice** for TensorStore chunking.
