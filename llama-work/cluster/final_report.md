...
The compression experiments on OpenLLaMA-3B and Llama-3.2-3B-Instruct reveal a complex trade-off between storage savings and performance overhead. Gzip compression achieves approximately 23 percent storage reduction across both models, reducing checkpoint sizes from roughly 6-7 GB to 5 GB. However, this storage benefit comes at a severe performance cost for save operations, with save times increasing by 78-216 percent depending on the model. OpenLLaMA-3B shows a 78 percent slowdown with compression, while Llama-3.2-3B-Instruct exhibits a more dramatic 216 percent slowdown. This substantial variation suggests that compression performance depends heavily on the specific weight distributions and parameter structures of each architecture.

Interestingly, load operations show slight improvements with compression, becoming 2-5 percent faster despite the need to decompress data. This counterintuitive result occurs because the reduced I/O volume (reading approximately 5 GB instead of 6-7 GB from disk) provides time savings that slightly outweigh the CPU decompression overhead. However, this benefit is minimal compared to the substantial save time penalty.

The compression results indicate that for local storage environments where disk space is abundant and I/O bandwidth is high, compression is generally counterproductive for saving operations. The CPU cycles spent compressing data far exceed any benefits from smaller files. However, the 23 percent storage reduction could be valuable in bandwidth-constrained scenarios such as cloud storage where network transfer costs dominate, or in distributed training environments where checkpoint synchronization across hundreds of machines benefits from reduced data volume. The slight load time improvement suggests compression might be acceptable for read-heavy workloads where checkpoints are loaded frequently but saved infrequently, such as inference deployments or fine-tuning scenarios.

### Concurrency Scaling Characteristics

The concurrency experiments tested five different concurrency levels (1, 4, 16, 64, and 128 concurrent operations) on Llama-3.2-3B-Instruct and OpenLLaMA-3B to determine the optimal parallelism for TensorStore operations. The results reveal unexpected non-monotonic scaling patterns where moderate concurrency levels can actually degrade performance compared to sequential operation. Both models show that increasing concurrency from 1 to 16 results in slower save times, with Llama-3.2-3B-Instruct degrading by 15 percent and OpenLLaMA-3B by 58 percent. Performance continues to vary at concurrency=64 before finally improving at very high concurrency (128), achieving modest gains of 18-35 percent for save operations compared to the sequential baseline.

This counterintuitive behavior suggests that moderate concurrency introduces overhead—such as thread scheduling costs, lock contention, and filesystem metadata bottlenecks—that outweighs the benefits of parallelization. At low concurrency, these overheads are minimal. At very high concurrency, the system achieves sufficient parallelism to overcome the overhead and realize net gains. The variation in degradation magnitude between Llama-3.2-3B-Instruct (15 percent) and OpenLLaMA-3B (58 percent) indicates that the optimal concurrency level depends on model-specific characteristics and their interaction with the storage subsystem.

Load operations demonstrate markedly different behavior, showing consistent improvement across all concurrency levels. Both models achieve substantial performance gains at high concurrency, with Llama-3.2-3B-Instruct improving by 74 percent and OpenLLaMA-3B by 85 percent at concurrency=128. This asymmetry between save and load operations indicates that read operations benefit more from parallelization than write operations, likely due to filesystem read-ahead caching and more efficient handling of concurrent read requests. The monotonic improvement in load times, contrasting with the non-monotonic save behavior, suggests that read parallelization avoids the contention issues observed during writes.

These findings have practical implications for TensorStore configuration. For write-intensive workloads, such as frequent checkpointing during training, low concurrency may be preferable to moderate levels, with very high concurrency (128 or higher) required to achieve performance benefits. For read-intensive workloads, such as loading checkpoints for inference or fine-tuning, higher concurrency consistently improves performance. The observed benefits at high concurrency for these 3B models suggest that parallelism is valuable across model sizes, not just for large-scale models.

### Chunk Size Optimization

The chunk size experiments tested six different chunk sizes (1, 4, 16, 64, 128, and 256 MB) across all four models to identify the optimal chunking strategy for TensorStore. The results reveal a clear optimal range with consistent patterns across models. Very small chunk sizes (1-4 MB) result in significantly degraded performance, while larger chunk sizes (64-256 MB) provide optimal and stable performance. Both save and load times improve dramatically as chunk size increases from 1 MB to 64 MB, then plateau with no further gains beyond 64 MB. This pattern holds consistently across OpenLLaMA-3B, Llama-3.2-3B-Instruct, Qwen2.5-7B, and Mistral-7B-v0.1 regardless of model size or architecture, indicating that **64 MB is a robust default choice** for TensorStore chunking.

Small chunk sizes create excessive overhead because each chunk requires separate metadata, file operations, and I/O calls. With 1 MB chunks, a large model is split into thousands of individual chunks, each requiring its own Zarr array metadata and file handle. This creates significant filesystem overhead and prevents efficient sequential I/O operations. Larger chunks reduce the number of separate operations and allow the storage system to perform more efficient sequential writes and reads.

The performance plateau at 64 MB represents an optimal balance where chunks are large enough to minimize metadata and filesystem overhead, but small enough to allow reasonable parallelization when high concurrency is employed. Beyond 64 MB, further increases provide no benefit because the chunks are already large enough to amortize the fixed costs of metadata and file operations. File sizes remain constant across all chunk size configurations, confirming that chunking is purely a storage organization strategy with no impact on total data volume.
