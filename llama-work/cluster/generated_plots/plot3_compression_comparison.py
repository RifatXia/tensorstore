#!/usr/bin/env python3
"""
generate compression comparison plots for multiple models
compares different compression types across different models
"""
...
# load all compression sweep data
compression_data = {}
model_full_names = {}  # store full model names
for sweep_dir in data_dir.iterdir():
    if sweep_dir.is_dir() and sweep_dir.name.startswith("202512") and "compression" in sweep_dir.name:
        # extract model from directory name
        model_id = sweep_dir.name.split('_')[-1]
        
        if model_id not in compression_data:
            compression_data[model_id] = {}
        
        # load both compression variants
        for comp_dir in sweep_dir.iterdir():
            if comp_dir.is_dir():
                results_file = comp_dir / "all_phases_results.json"
                if results_file.exists():
                    with open(results_file, 'r') as f:
                        data = json.load(f)
                        compression_type = data['compression']
                        compression_data[model_id][compression_type] = data
                        # store full model name
                        model_full_names[model_id] = data['model_name']

print(f"loaded {len(compression_data)} models")
for model_id, comp_types in compression_data.items():
    print(f"  {model_id}: {sorted(comp_types.keys())}")

# ============================================================================
# compression comparison plot (save time, load time, and file size)
# ============================================================================
if len(compression_data) >= 2:
    fig, axes = plt.subplots(1, 3, figsize=(20, 7))
    fig.suptitle('Compression Impact: Model Comparison', fontsize=16, fontweight='bold', y=0.98)
    
    models = list(compression_data.keys())[:2]  # take first 2 models
    model_colors = ['#3498db', '#e74c3c']
    
    # get all compression types (sorted)
    all_compression_types = set()
    for model_id in models:
        all_compression_types.update(compression_data[model_id].keys())
    compression_types = sorted(all_compression_types)
    ...
    for model_id in models:
        ...
        for comp_type in compression_types:
            if comp_type in compression_data[model_id]:
                data = compression_data[model_id][comp_type]
    ...
    x_pos = np.arange(len(compression_types))
    ...
    ax_save.set_xlabel('compression type', fontsize=14, fontweight='bold')
    ax_save.set_ylabel('time (s)', fontsize=14, fontweight='bold')
    ax_save.set_xticks(x_pos)
    ax_save.set_xticklabels(compression_types, fontsize=12, fontweight='bold')
    ...
    ax_load.set_xlabel('compression type', fontsize=14, fontweight='bold')
    ax_load.set_ylabel('time (s)', fontsize=14, fontweight='bold')
    ax_load.set_xticks(x_pos)
    ax_load.set_xticklabels(compression_types, fontsize=12, fontweight='bold')
    ...
    ax_size.set_xlabel('compression type', fontsize=14, fontweight='bold')
    ax_size.set_ylabel('file size (gb)', fontsize=14, fontweight='bold')
    ax_size.set_xticks(x_pos)
    ax_size.set_xticklabels(compression_types, fontsize=12, fontweight='bold')
    ...
    config_text = (
        ...
        "• Concurrency: 128\n"
        "• Device: CPU\n"
        "• Variable: Compression (none, gzip)"
    )
    ...
    output_file = data_dir / "compression_comparison.png"
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    print(f"\n✓ compression comparison saved: {output_file}")
    plt.close()
    ...
    for idx, model_id in enumerate(models):
        ...
        full_name = model_full_names.get(model_id, model_id)
        offset = (idx - 0.5) * width
        bars = ax.bar(x_pos + offset, save_times, width,
                     label=full_name, color=model_colors[idx], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            if height > 0:
    ...
    ax.set_xlabel('Compression Type', fontsize=14, fontweight='bold')
    ax.set_ylabel('Time (s)', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ax.set_xticklabels(compression_types, fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    save_file = data_dir / "compression_comparison_save.png"
    ...
    for idx, model_id in enumerate(models):
        ...
        full_name = model_full_names.get(model_id, model_id)
        offset = (idx - 0.5) * width
        bars = ax.bar(x_pos + offset, load_times, width,
                     label=full_name, color=model_colors[idx], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            if height > 0:
    ...
    plt.tight_layout()
    load_file = data_dir / "compression_comparison_load.png"
    ...
    for idx, model_id in enumerate(models):
        ...
        full_name = model_full_names.get(model_id, model_id)
        offset = (idx - 0.5) * width
        bars = ax.bar(x_pos + offset, file_sizes, width,
                     label=full_name, color=model_colors[idx], alpha=0.8)
        for bar in bars:
            height = bar.get_height()
            if height > 0:
    ...
    plt.tight_layout()
    size_file = data_dir / "compression_comparison_size.png"
    plt.savefig(size_file, dpi=150, bbox_inches='tight')
    print(f"✓ file size plot saved: {size_file}")
    plt.close()

print("\n" + "="*70)
print("✓ compression comparison plot generated successfully!")
print("="*70)
print(f"\noutput files:")
print(f"  - {data_dir}/compression_comparison.png (combined)")
print(f"  - {data_dir}/compression_comparison_save.png (save time)")
print(f"  - {data_dir}/compression_comparison_load.png (load time)")
print(f"  - {data_dir}/compression_comparison_size.png (file size)")
print(f"\nmodels compared: {', '.join([model_full_names.get(m, m) for m in models])}")
print(f"compression types: {', '.join(compression_types)}")
