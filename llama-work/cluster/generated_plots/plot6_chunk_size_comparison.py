...
# ============================================================================
if len(chunk_data) >= 2:
    fig, axes = plt.subplots(1, 3, figsize=(32, 7))
    ...
    ax_save.set_xlabel('chunk size (mb)', fontsize=14, fontweight='bold')
    ax_save.set_ylabel('time (s)', fontsize=14, fontweight='bold')
    ax_save.set_xticks(x_pos)
    ax_save.set_xticklabels(chunk_values, fontsize=12, fontweight='bold')
    ...
    ax_load.set_xlabel('chunk size (mb)', fontsize=14, fontweight='bold')
    ax_load.set_ylabel('time (s)', fontsize=14, fontweight='bold')
    ax_load.set_xticks(x_pos)
    ax_load.set_xticklabels(chunk_values, fontsize=12, fontweight='bold')
    ...
    ax_size.set_xlabel('chunk size (mb)', fontsize=14, fontweight='bold')
    ax_size.set_ylabel('file size (gb)', fontsize=14, fontweight='bold')
    ax_size.set_xticks(x_pos)
    ax_size.set_xticklabels(chunk_values, fontsize=12, fontweight='bold')
    ...
    # save time only
    fig_save = plt.figure(figsize=(16, 7))
    ...
    ax.set_xlabel('Chunk Size (MB)', fontsize=14, fontweight='bold')
    ax.set_ylabel('Time (s)', fontsize=14, fontweight='bold')
    ax.set_xticks(x_pos)
    ax.set_xticklabels(chunk_values, fontsize=12, fontweight='bold')
    ax.tick_params(axis='y', labelsize=12)
    ax.ticklabel_format(style='plain', axis='y')
    for label in ax.get_yticklabels():
        label.set_fontweight('bold')
    ax.legend(fontsize=11, loc='upper right', bbox_to_anchor=(1, 1), framealpha=0.9)
    ax.grid(axis='y', alpha=0.3)
    ax.set_ylim(top=max([max(all_data[m]['save_times']) for m in models]) * 1.3)
    plt.tight_layout()
    save_file = data_dir / "chunk_size_comparison_save.png"
    plt.savefig(save_file, dpi=150, bbox_inches='tight')
    print(f"✓ save time plot saved: {save_file}")
    plt.close()
    
    # load time only
    fig_load = plt.figure(figsize=(16, 7))
    ...
    plt.tight_layout()
    size_file = data_dir / "chunk_size_comparison_size.png"
...
print("="*70)
print(f"\noutput files:")
print(f"  - {data_dir}/chunk_size_comparison.png (combined)")
print(f"  - {data_dir}/chunk_size_comparison_save.png (save time)")
print(f"  - {data_dir}/chunk_size_comparison_load.png (load time)")
print(f"  - {data_dir}/chunk_size_comparison_size.png (file size)")
